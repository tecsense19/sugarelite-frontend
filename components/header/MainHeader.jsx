        ...
        <div className="flex justify-between items-center w-full">
          ...
          <div className="flex items-center me-[72px]">
            <div className="flex flex-row gap-x-[30px] me-[35px]">
              ...
              <Link href={client_routes.chat} className="flex">
              ...
              <Link href={client_routes.search} className="py-[7px] rounded-[5px] h-[32px] flex items-center">
                <Image height={18} width={18} src={search} className="" alt="" priority />
                {/* <input type="text" placeholder="Search" className="w-full bg-transparent outline-none border-0 text-[12px] font-light" style={{ lineHeight: "normal" }} /> */}
              </Link>
            </div>
            <button className="h-8 w-[78px] me-[35px] rounded-[5px] flex items-center justify-center bg-secondary text-[12px] font-semibold leading-[normal]" onClick={handleLogout}>
              Logout
            </button>
            {
              (userState?.user)
                ? <Link href={client_routes.profile}>
                ...
                : <Link href={client_routes.profile}>
                  {user?.avatar_url
                    ? <Image height={40} width={40} src={user?.avatar_url} alt="profile_pic" className="cursor-pointer" priority onClick={() => router.push(client_routes.profile)} />
