...
const RootHeader = ({ user, allUsers, matchNotifications, albumNotifications, chatList }) => {
    ...
    const { addMessage, updateOnlineUsers } = useChat()
    ...
    useEffect(() => {
        if (user) {
            ...
            newSocket.on("onlineUsers", (arr) => {
                const filteredArr = arr.filter(i => i.userId !== user.id)
                updateOnlineUsers(filteredArr)
            });

            newSocket.on("receive-msg", (msg) => {
                addMessage(msg);
            });

            newSocket.on('opened-chat', (arr) => {
                const myPartnerList = arr.filter(i => i.receiverId === user.id)
                dispatch({ type: "Add_Partner", payload: myPartnerList })
            })

            newSocket.on('closed-chat', (arr) => {
                const myPartnerList = arr.filter(i => i.receiverId === user.id)
                dispatch({ type: "Add_Partner", payload: myPartnerList })
            })
            ...
            newSocket.on("blocked-status", blockUserHandler);
            newSocket.on("unblocked-status", unblockUserHandler);
            newSocket.on("album-notification", albumAccessHandler);
            newSocket.on('swipe-notify', swipeHandler)
        }
        ...
        return () => {
            ...
            if (currentSocket) {
                ...
                currentSocket.off('send-msg')
                currentSocket.off('opened-chat')
                currentSocket.off('closed-chat')
                currentSocket.off("blocked-status",);
                currentSocket.off("unblocked-status",);
                currentSocket.off("album-notification",);
                currentSocket.off('swipe-notify')
    ...
    return (
        <>
            ...
            <NotificationComaponent open={notificationOpenState} allUsers={allUsers} user={user} />
