...
const Msg = ({ msg, setSelectedImages }) => {
    if (msg.get_all_chat_with_image?.length) {
        return (
            <div className=' flex flex-col gap-2 max-w-[12rem] w-full'>

                {/* <p className=''>{msg.text}</p> */}
                <div className={`overflow-hidden relative flex gap-1 justify-between flex-wrap cursor-pointer ${msg.get_all_chat_with_image.length === 2 ? "h-[6rem] w-full min-w-[12rem]" : "w-full min-w-[12rem] h-[12rem]"}`} onClick={() => setSelectedImages(msg.get_all_chat_with_image)}>
                    {
