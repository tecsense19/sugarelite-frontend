              ...
              <div className="">
                <div className="flex items-center">
                  <div className="text-[clamp(16px,2vw,22px)] lg:text-[clamp(19px,2vw,22px)] leading-[normal] lg:leading-[30px] font-bold">
                    <span className="hidden sm:inline">{item.name}</span><span className="sm:hidden inline">{item.name.split(' ')[0]}</span>,{item.age}
                  </div>
                  {item.is_premium && <Image src={premiumUserIcon} alt="" height={22} width={22} priority className="ms-2 pointer-events-none" />}
                </div>
                <div className="mt-1 md:mt-[2px] text-[clamp(12px,1.5vw,16px)]  lg:text-[clamp(14px,1.5vw,16px)] leading-[14px] font-semibold text-white/50">{item.desc}</div>
              </div>
            </div>
          </div>
        )
