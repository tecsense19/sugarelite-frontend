...
//     const [user, setUsers] = useState(users.slice(users.length - 10, users.length))

//     const handleSwipe = (d, profile) => {
//         if (d === "left" || d === "right") {
//             console.log(d)
//             if (profile.id === 65) {
//                 setUsers(users.slice(users.length - 20, users.length - 10))
//             }
...
const Tinder = ({ users }) => {
    ...
    return (
        <div ref={swipeRef} className="h-dvh flex relative justify-center items-center">
            {users.map((user, index) => (
                <div
                    key={index}
                    className="card h-[80%] w-[80%] absolute bg-red-400"
                    style={{
