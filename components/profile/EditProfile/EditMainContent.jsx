...
const EditMainContent = ({ control, progress, publicPhotoList, setPublicPhotoList, privatePhotoList, setPrivatePhotoList, user, setRemovalArray, isLoading }) => {
        ...
        <div className="w-full lg:ml-[350px] 2xl:ml-[400px] text-white mt-[40px] px-[15px] lg:mt-[30px] lg:px-[50px]" >
            ...
            <div className="border-white lg:border-b border-opacity-20 lg:pb-[40px]" data-aos='zoom-in'>
                ...
                <div className="mt-[20px] lg:mt-[25px]">
                    <UploadPic progress={progress} control={control} name="public_photos" photoList={publicPhotoList} setPhotoList={setPublicPhotoList} uploadedImages={user.get_all_profileimg && user.get_all_profileimg.filter((i) => i.image_type === "public")} setRemovalArray={setRemovalArray} />
                </div>
            </div>
            <div className="border-white lg:border-b border-opacity-20 pt-[40px] lg:pb-[40px]  w-full" data-aos='zoom-in'>
                ...
                <div className="mt-[20px] lg:mt-[25px]">
                    <UploadPic progress={progress} control={control} name="private_photos" photoList={privatePhotoList} setPhotoList={setPrivatePhotoList} uploadedImages={user.get_all_profileimg && user.get_all_profileimg.filter((i) => i.image_type === "private")} setRemovalArray={setRemovalArray} />
                </div>
            </div>
                ...
                <div className="mt-[25px] grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 gap-[20px] lg:gap-[30px]">
                    {
                        profile_select_options_appearance.map((type, inx) => {
                            return <SelectBox name={type.name} options={type.options} text={type.text} key={inx} control={control} inx={inx} user={user} />
                        })
                    }
                </div>
            </div>
