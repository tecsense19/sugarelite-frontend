...
const SelectBox = ({ name, options, text, control, inx, user, allStrings }) => {
    ...
    return (
        <div className="relative aspect-auto profile" data-aos={animation} data-aos-duration="800">
            <Controller
                name={name}
                control={control}
                defaultValue={user[name] ? user[name] : allStrings["string_ask_me"]}
                render={({ field }) => (
                    <ConfigProvider
                        theme={customStyles}
                    >
                        <Select
                            {...field}
                            placeholder={allStrings["string_ask_me"]}
